...
huffman_decode(int16_t (*dst)[8][8], const uint8_t *src, int count, int maxlen)
{
	...
	for (i=0; i<count; i++) {
		...
		/*
		printf("DC info: %08x, cat=%d, sign=%d, extra=%d, coeff=%d\n",
				dc_info, dc_category, dc_sign, dc_extra_bits, dc_coeff);
		*/
